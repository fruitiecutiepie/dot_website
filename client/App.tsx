...
class App extends React.Component<any, IState> {
  ...
  constructor(props: any) {
    ...
    this.onSearchBarActionInvoked = this.onSearchBarActionInvoked.bind(this)
    this.onViewportActionInvoked = this.onViewportActionInvoked.bind(this)
    ...
    this.connection.on('extension.openFindSearchBar', (result: any) => {
      this.setState({
        isSearchEnabled: true,
      })
    })

    this.connection.on('extension.pinnedUrl', (result: any) => {
      this.updateState({
        pinnedUrl: result.pinnedUrl,
      })
      this.handlePinnedUrl();
    });
    ...
    this.connection.on('extension.contextMenu', (result: any) => {
      console.log('contextMenu', result)
      this.viewport.contextMenuRef.current?.setState({
        isVisible: true,
        position: {
          x: result.x,
          y: result.y,
    ...
    this.connection.on('extension.click', (result: any) => {
      this.viewport.contextMenuRef.current?.setState({
        isVisible: false,
      })
    })
    ...
    public render() {
      return (
        <div className="App">
        {
          ...
          : (
            ...
            <Toolbar
              ...
              pinnedUrl={this.state.pinnedUrl}
            />
            {this.state.isSearchEnabled
            ? <SearchBar
              onActionInvoked={this.onSearchBarActionInvoked}
            />
            : null}
            </>
          )
        ...
        <Viewport
          ...
          url={this.state.url}
          onActionInvoked={this.onViewportActionInvoked}
  ...
  private async requestNavigationHistory() {
    ...
    // this.connection.send('extension.updateLastUrlSourceDocument', {
    //   url
    // })
  }
  ...
  private async onViewportChanged(action: string, data: any) {
    switch (action) {
      case 'inspectHighlightRequested':
        await this.handleInspectHighlightRequested(data)
        break
      case 'inspectElement':
...
// search bar feature
  private async onSearchBarActionInvoked(action: string, data: any): Promise<any> {
    switch (action) {
      case 'findSearchBarQuery':
        if (!data || !data.text) {
          console.error(`extension.findSearchBarQuery: data ${JSON.stringify(data)}`)
          return
        }
        return await this.connection.send('extension.findSearchBarQuery', {
          value: {
            text: data.text
          }
        })
      case 'scrollToFindSearchBarQueryMatch':
        if (!data || data.previousIndex === undefined || data.currentIndex === undefined) {
          console.error(`extension.scrollToFindSearchBarQueryMatch: data ${JSON.stringify(data)}`)
          return
        }
        this.connection.send('extension.scrollToFindSearchBarQueryMatch', {
          value: {
  ...
  private async onViewportActionInvoked(action: string, data: any): Promise<any> {
    switch (action) {
      ...
      case 'removeSelection':
        if (!data || !data.value) {
          console.error(`Page.removeSelection: data ${JSON.stringify(data)}`)
          return
        }
        this.connection.send('Page.removeSelection', data)
        break
      case 'pasteSelection':
        if (!data || !data.value) {
          console.error(`Page.pasteSelection: data ${JSON.stringify(data)}`)
          return
        }
        this.connection.send('Page.pasteFromClipboard', data)
        break
      case 'selectAll':
        if (!data || !data.value) {
          console.error(`Page.selectAll: data ${JSON.stringify(data)}`)
          return
        }
        this.connection.send('Page.selectAll', data)
        break
    }
    // return an empty promise
    return Promise.resolve()
  ...
  private handlePinnedUrl() {
    if (this.state.pinnedUrl) {
      this.handleNavigate(this.state.pinnedUrl)
    }
  }
